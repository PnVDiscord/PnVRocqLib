...
CoInductive itree (E : Type@{U_discourse} -> Type@{U_discourse}) (R : Type@{U_discourse}) : Type@{U_discourse} :=
  go { observe : itreeF (itree E R) E R } as t.

#[global] Arguments go {E}%_type_scope {R}%_type_scope observe.
#[global] Arguments observe {E}%_type_scope {R}%_type_scope t.
...
Instance itree_isMonad : isMonad@{U_discourse U_discourse} (itree E) :=
  { pure {A : Type@{U_discourse}} (x : A) := Ret x
  ; bind {A : Type@{U_discourse}} {B : Type@{U_discourse}} (m : itree E A) (k : A -> itree E B) := itree_bind' (R1 := A) (R2 := B) k m
  }.

#[global]
Instance itree_isMonadIter : isMonadIter (itree E) :=
  fun I : Type => fun R : Type => fun step : I -> itree E (I + R)%type =>
...
Definition itree_rec {E : handlerCat.(CAT.ob)} {I : Type} {R : Type} (body : I -> itree (callE I R +' E) R) (arg : I) : itree E R :=
  itree_mrec (E := callE I R) (E' := E) (callE_handler body) R (Call arg).

Definition itree_call {E : handlerCat.(CAT.ob)} {I : Type} {R : Type} (arg : I) : itree (callE I R +' E) R :=
  handlerCat_hasCoproduct.(CAT.inl) R (Call arg).

Definition itree_rec_fix {E : handlerCat.(CAT.ob)} {I : Type} {R : Type} (body : endo (I -> itree (callE I R +' E) R)) : I -> itree E R :=
