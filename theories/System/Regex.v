...
Theorem pumping_lemma (e : regex A) (s : list A)
  ...
  : pumping e s.
Proof.
  induction H_in as [ | c | s e1 e2 H_INL IHL | s e1 e2 H_INR IHR | s1 s2 e1 e2 H_IN1 IH1 H_IN2 IH2 | e1 | e1 s1 s2 H_IN1 IH1 H_IN2 IH2]; simpl in *; try lia; s!.
  ...
  - assert (pumping_constant e1 <= length s1 \/ pumping_constant e1 > length s1) as [H_le1 | H_gt1] by lia; [ | assert (pumping_constant e2 <= length s2 \/ pumping_constant e2 > length s2) as [H_le2 | H_gt2] by lia]; try lia.
    + specialize (IH1 H_le1). destruct IH1 as [s1' s2' s3' ? ? ? ?]. exists s1' s2' (s3' ++ s2); try using eqn: H_split. i. replace (s1' ++ s2' ^ m ++ s3' ++ s2) with ((s1' ++ s2' ^ m ++ s3') ++ s2) by done!. eauto.
