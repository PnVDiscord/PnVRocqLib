...
Record expnOrderType {alpha : Tree} {beta : Tree} : Type :=
  { call : ToOrderType beta -> ToOrderType alpha
  ; supp : list (ToOrderType beta)
  ; supp_inc : forall i : nat, forall j : nat, i < j -> forall xi_i, forall xi_j, nth_error supp i = Some xi_i -> nth_error supp j = Some xi_j -> xi_i ≺ xi_j
  ; supp_fin : forall xi : ToOrderType beta, L.In xi supp <-> ~ fromOrderType (ToOrderType alpha) (call xi) == empty
  ; call_eqPropCompatible : eqPropCompatible1 call
  } as expnOrderType.

#[global] Arguments expnOrderType : clear implicits.

#[global] Existing Instance call_eqPropCompatible.

Section EXPN_DEFN.

Context (alpha : Tree) (beta : Tree).

#[global]
Instance expnOrderType_isSetoid : isSetoid (expnOrderType alpha beta) :=
  { eqProp (lhs : expnOrderType alpha beta) (rhs : expnOrderType alpha beta) := forall xi : ToOrderType beta, lhs.(call) xi == rhs.(call) xi
  ; eqProp_Equivalence := relation_on_image_liftsEquivalence (pi_isSetoid (fun _ : ToOrderType beta => ToOrderType_isSetoid alpha)).(eqProp_Equivalence) call
  }.

Variant expnOrderType_wlt (f : expnOrderType alpha beta) (g : expnOrderType alpha beta) : Prop :=
  | expnOrderType_wlt_intro (delta : ToOrderType beta)
    (wlt_at_delta : f.(call) delta ≺ g.(call) delta)
    (agree_after_delta : forall xi : ToOrderType beta, forall H_wlt : delta ≺ xi, f.(call) xi == g.(call) xi)
    : expnOrderType_wlt f g.

#[global]
Instance expnOrderType_wlt_eqPropCompatible2
  : eqPropCompatible2 expnOrderType_wlt.
Proof.
  ii. do 2 red in x_EQ, y_EQ. change (expnOrderType_wlt x1 y1 <-> expnOrderType_wlt x2 y2).
  split; intros [delta ? ?]; exists delta.
  - now rewrite <- x_EQ, <- y_EQ.
  - ii. rewrite <- x_EQ, <- y_EQ; done!.
  - now rewrite -> x_EQ, -> y_EQ.
  - ii. rewrite -> x_EQ, -> y_EQ; done!.
Qed.

#[global]
Instance expnOrderType_wlt_Transitive
  : Transitive expnOrderType_wlt.
Proof.
  intros f g h [xi1 xi1_disagree xi1_max] [xi2 xi2_disagree xi2_max].
  pose proof (O.wlt_trichotomous (classic := classic) xi1 xi2) as [xi1_eq_xi2 | [xi1_lt_xi2 | xi2_lt_xi1]]; cycle 1.
  - assert (claim1 : call f xi2 == call g xi2).
    { eapply NNPP. intros H_disagree. contradiction (H_disagree (xi1_max _ xi1_lt_xi2)). }
    exists xi2.
    + now rewrite -> claim1.
    + ii. rewrite -> xi1_max; cycle 1. { transitivity xi2; eauto. } now eapply xi2_max.
  - assert (claim2 : call g xi1 == call h xi1).
    { eapply NNPP. intros H_disagree. contradiction (H_disagree (xi2_max _ xi2_lt_xi1)). }
    exists xi1.
    + now rewrite <- claim2.
    + ii. rewrite <- xi2_max; cycle 1. { transitivity xi1; eauto. } now eapply xi1_max.
  - exists xi1.
    + transitivity (call g xi1); eauto. rewrite xi1_eq_xi2; eauto.
    + ii. transitivity (call g xi); eauto. eapply xi2_max. now rewrite <- xi1_eq_xi2.
Qed.
