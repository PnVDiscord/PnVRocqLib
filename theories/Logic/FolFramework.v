...
Inductive frm : Set :=
  ...
  | scheme_app {n : arity} {xs : Vector.t name n} (phi : scheme xs) (ts : Vector.t trm n) : frm.
...
Inductive Similarity_scheme {n : arity} (xs' : Vector.t name n) : Similarity (Vector.t (InternalSyntax.trm L) n -> InternalSyntax.frm L) (scheme xs') :=
  | mk_scheme_corres (xs : Vector.t ivar n) phi phi'
    (xs_corres : xs =~= xs')
    (phi_corres : forall ts : Vector.t (InternalSyntax.trm L) n, alpha_equiv (phi ts) (subst_frm (mk_subst xs ts) phi'.(scheme_body xs')))
    : phi =~= phi'.
...
Inductive Similarity_frm : Similarity (InternalSyntax.frm L) frm :=
  ...
  | scheme_app_corres (n : arity) (xs' : Vector.t name n) (ts : Vector.t (InternalSyntax.trm L) n) (ts' : Vector.t trm n) (phi : Vector.t (InternalSyntax.trm L) n -> InternalSyntax.frm L) (phi' : scheme xs')
    (ts_corres : ts =~= ts')
    (phi_corres : phi =~= phi')
